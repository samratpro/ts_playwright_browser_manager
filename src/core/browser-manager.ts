...
export class BrowserManager {
  ...
  private debugLog(...args: any[]) {
    if (this.debug) console.debug('[BrowserManager]', ...args);
  }
  ...
  private playwrightBrowser: Browser | undefined;
  private processPid: number | undefined;

  constructor(options: BrowserManagerOptions = {}) {
    if ((options as any).debug) this.debug = true;
  ...
  private findBrowserPath(): string {
    ...
    const possiblePaths: BrowserPath[] = [];
    ...
    if (platform === 'darwin') {
      possiblePaths.push(
        ...
        { type: 'brave', path: path.join(os.homedir(), 'Applications/Brave Browser.app/Contents/MacOS/Brave Browser') },
        { type: 'comet', path: '/Applications/Comet Browser.app/Contents/MacOS/Comet Browser' },
        { type: 'comet', path: path.join(os.homedir(), 'Applications/Comet Browser.app/Contents/MacOS/Comet Browser') },
        { type: 'edge', path: '/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge' },
        { type: 'edge', path: path.join(os.homedir(), 'Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge') },
        { type: 'chrome', path: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome' },
        { type: 'chrome', path: path.join(os.homedir(), 'Applications/Google Chrome.app/Contents/MacOS/Google Chrome') },
        { type: 'chromium', path: '/Applications/Chromium.app/Contents/MacOS/Chromium' },
        { type: 'chromium', path: path.join(os.homedir(), 'Applications/Chromium.app/Contents/MacOS/Chromium') }
      );
    }
    ...
    if (platform === 'win32') {
      possiblePaths.push(
        { type: 'brave', path: 'C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe' },
        { type: 'brave', path: 'C:\\Program Files (x86)\\BraveSoftware\\Brave-Browser\\Application\\brave.exe' },
        { type: 'brave', path: path.join(os.homedir(), 'AppData\\Local\\BraveSoftware\\Brave-Browser\\Application\\brave.exe') },
        { type: 'comet', path: 'C:\\Program Files\\CometBrowser\\Application\\comet.exe' },
        { type: 'comet', path: 'C:\\Program Files (x86)\\CometBrowser\\Application\\comet.exe' },
        { type: 'comet', path: path.join(os.homedir(), 'AppData\\Local\\CometBrowser\\Application\\comet.exe') },
        ...
        { type: 'edge', path: path.join(os.homedir(), 'AppData\\Local\\Microsoft\\Edge\\Application\\msedge.exe') },
        ...
        { type: 'chrome', path: 'C:\\Program Files\\Google\\Chrome Beta\\Application\\chrome.exe' },
        { type: 'chrome', path: 'C:\\Program Files\\Google\\Chrome Canary\\Application\\chrome.exe' },
        { type: 'chromium', path: 'C:\\Program Files\\Chromium\\Application\\chromium.exe' },
        { type: 'chromium', path: 'C:\\Program Files (x86)\\Chromium\\Application\\chromium.exe' },
        { type: 'chromium', path: path.join(os.homedir(), 'AppData\\Local\\Chromium\\Application\\chromium.exe') }
      );
    }
    ...
    if (platform === 'linux') {
      possiblePaths.push(
        { type: 'brave', path: '/usr/bin/brave-browser' },
        { type: 'brave', path: '/usr/bin/brave' },
        { type: 'brave', path: '/usr/local/bin/brave-browser' },
        { type: 'brave', path: '/usr/local/bin/brave' },
        { type: 'brave', path: path.join(os.homedir(), '.local/bin/brave-browser') },
        { type: 'comet', path: '/usr/bin/comet-browser' },
        { type: 'comet', path: '/usr/bin/comet' },
        { type: 'comet', path: '/usr/local/bin/comet-browser' },
        { type: 'comet', path: '/usr/local/bin/comet' },
        { type: 'comet', path: path.join(os.homedir(), '.local/bin/comet-browser') },
        { type: 'edge', path: '/usr/bin/microsoft-edge' },
        { type: 'edge', path: '/usr/bin/microsoft-edge-stable' },
        { type: 'edge', path: '/usr/local/bin/microsoft-edge' },
        { type: 'chrome', path: '/usr/bin/google-chrome' },
        { type: 'chrome', path: '/usr/bin/google-chrome-stable' },
        { type: 'chrome', path: '/usr/local/bin/google-chrome' },
        { type: 'chromium', path: '/usr/bin/chromium' },
        { type: 'chromium', path: '/usr/bin/chromium-browser' },
        { type: 'chromium', path: '/usr/local/bin/chromium' },
        { type: 'chromium', path: path.join(os.homedir(), '.local/bin/chromium') }
      );
    }
    ...
    throw new Error(
      'No supported browser found in standard locations.\n' +
   ...
   */
  private async killBrowserProcess(pid: number): Promise<void> {
    try {
      this.debugLog(`Killing browser process: ${pid}`);
      await new Promise<void>((resolve) => {
        kill(pid, (error: any) => {
          if (error) {
            console.warn(`Warning: Could not kill process ${pid}: ${error && (error as Error).message}`);
          } else {
   ...
   */
  public getProfilePath(profileName: string): string {
  ...
  public async setupProfile(
    profileName: string,
    url?: string,
    waitMessage: string = 'Perform manual actions, then close the browser to save.',
   ...
   */
  public async connectToBrowserWithProxy(
    options: ProxyConnectionOptions
  ): Promise<BrowserConnectionResult> {
   ...
   */
  public async connectToBrowserAsync(
    profileName: string,
    url?: string,
    headless: boolean = false,
    timeout: number = 60000
   ...
   */
  public async connectToBrowserAsyncWithProxy(
    options: ProxyConnectionOptions
  ): Promise<BrowserConnectionResult> {
   ...
   */
  public async closeBrowserAsync(): Promise<void> {
