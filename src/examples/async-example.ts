...
import type { AsyncScrapeResult } from '../types/index.js';
...
async function useCase1_MultipleTabsSameProfile() {
  ...
  try {
    ...
    const { context } = await manager.connectToBrowser(
      profileName,
    ...
    const scrapePage = async (page: any, name: string) => {
...
async function useCase2_MultipleProfilesMultipleTabs() {
  ...
  const manageProfile = async (profileName: string, debugPort: number, urls: string[]) => {
    const manager = new BrowserManager({ debugPort });
  ...
  const tasks = profilesConfig.map(config =>
    manageProfile(config.profile, config.port, config.urls)
  );
...
async function useCase3_ParallelScraping() {
  ...
  const scrapeSingleLink = async (context: any, link: string, maxConcurrent: number): Promise<AsyncScrapeResult> => {
  ...
  const scrapeWithSemaphore = async (maxConcurrent: number) => {
    const debugPort = 9221;
    const profileName = 'my_facebook_profile';
    const manager = new BrowserManager({ debugPort });
...
async function useCase4_DynamicTabManagement() {
  ...
  try {
    ...
    const itemsToCheck = Array.from({ length: 10 }, (_, i) => 
      `https://www.facebook.com/marketplace/item/${i + 1}`
    );
